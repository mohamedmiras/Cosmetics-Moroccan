...
const activeProduct = {
  id: "crimson-rose",
  name: "Crimson Rose Cream",
  description: "Infused with botanical extracts and crushed rose petals. Instantly restores vitality for a luminous finish.",
  price: 6,
  image: "/nobg_products/no-bg-Red powder.webp",
  tag: "Exclusive"
};
...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(imageRef.current, 
        ...
        { y: 0, opacity: 1, scale: 1, rotation: 0, duration: 0.9, ease: "power2.out" }
      );
  ...
  return (
    <div ref={containerRef} className="w-full bg-transparent dark:bg-[#0a0202] flex items-center justify-center py-4 md:py-6 px-8 relative overflow-hidden transition-colors duration-500">
      
      {/* Animated Flowing Light/Dark Background */}
      <div className="absolute inset-0 w-full h-full pointer-events-none product-glow">
        <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-white/40 dark:bg-red-900/20 rounded-full blur-[80px]"></div>
        <div className="absolute bottom-1/4 right-1/4 w-[300px] h-[300px] bg-white/30 dark:bg-rose-800/10 rounded-full blur-[80px]"></div>
      </div>

      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-4 items-center z-10">
        
        <div className="relative mb-4 md:mb-0 h-[280px] md:h-[520px] w-full flex justify-center items-center">
          
          {/* Image Container without Mask (since image has no background) */}
          <div className="absolute inset-0 w-full h-full overflow-visible">
            <div className="w-full h-full flex justify-center items-center transform-gpu will-change-[transform,opacity]" ref={imageRef}>
              <div className="product-image-wrap w-full h-full flex justify-center items-center animate-[float_5s_ease-in-out_infinite] transform-gpu">
                <img 
                  ...
                  loading="eager"
                  className="w-full h-full scale-[1.5] md:scale-[1.7] dark:scale-[1.6] dark:md:scale-[1.8] object-contain origin-center transform-gpu md:hover:scale-[1.8] dark:md:hover:scale-[1.9]"
                />
              </div>
            </div>
          </div>
        ...
        <div ref={textGroupRef} className="flex flex-col items-center text-center md:items-start md:text-left -mt-6 md:-mt-8 transform-gpu will-change-[transform,opacity]">
          <p className="text-[#a44242] dark:text-red-500 font-mono tracking-[0.2em] uppercase text-xs mb-2 transition-colors duration-500">
            {activeProduct.tag}
          </p>
          ...
          <div className="flex items-end gap-6 mb-4 justify-center md:justify-start">
            ...
            <span className="text-sm font-mono text-[#5a443f]/60 dark:text-white/40 mb-1 tracking-widest uppercase transition-colors duration-500">/ 100ml</span>
          </div>
