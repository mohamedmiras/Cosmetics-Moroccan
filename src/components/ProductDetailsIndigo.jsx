...
const activeProduct = {
  id: "savon-noir-a-nila",
  name: "Savon Noir À Nila",
  description: "A masterful fusion of traditional Moroccan black soap and brightening Nila powder. Deeply purifies while leaving the skin glowing and radiantly refreshed.",
  price: 6,
  image: "/nobg_products/Savor_Noir_Nila_nobg.webp",
  imageLight: "/Videos/product_visuals/Savor_Noir_Nila_hd.webp",
  tag: "Purifying & Brightening"
};

const ProductDetailsIndigo = () => {
  ...
  return (
    <div ref={containerRef} className="w-full bg-transparent dark:bg-[#070b19] flex items-center justify-center py-4 md:py-6 px-8 relative overflow-hidden transition-colors duration-500">
      
      {/* Animated Flowing Light/Dark Background */}
      <div className="absolute inset-0 w-full h-full pointer-events-none product-glow">
        <div className="absolute top-1/3 right-1/4 w-[500px] h-[500px] bg-white/40 dark:bg-indigo-900/10 rounded-full blur-[80px]"></div>
        <div className="absolute bottom-1/3 left-1/4 w-[400px] h-[400px] bg-white/30 dark:bg-blue-900/10 rounded-full blur-[80px]"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-4 items-center z-10">
        ...
        <div className="relative mb-4 md:mb-0 h-[280px] md:h-[520px] w-full flex justify-center items-center transform-gpu will-change-[transform,opacity]" ref={imageRef}>
          ...
          {/* Light Theme Version: Soft Background, Masked, Enlarged Size */}
          <div 
            className="absolute inset-0 w-full h-full flex justify-center items-center opacity-100 dark:opacity-0 transition-opacity duration-700"
            style={{
          ...
          {/* Dark Theme Version: No Background, No Mask, Massive Size */}
          <div className="absolute inset-0 w-full h-full flex justify-center items-center opacity-0 dark:opacity-100 transition-opacity duration-700">
            <div className="product-image-wrap w-full h-full flex justify-center items-center animate-[float_5s_ease-in-out_infinite] transform-gpu">
              <img 
                src={activeProduct.image} 
                alt={activeProduct.name} 
                loading="lazy"
                className="w-full h-full scale-[1.7] md:scale-[1.9] object-contain origin-center transform-gpu md:hover:scale-[2.0]"
              />
            </div>
          </div>
        ...
        <div ref={textGroupRef} className="flex flex-col items-center text-center md:items-start md:text-left -mt-6 md:-mt-8 transform-gpu will-change-[transform,opacity]">
          <p className="text-indigo-700 dark:text-indigo-400 font-mono tracking-[0.2em] uppercase text-xs mb-2 transition-colors duration-500">
            {activeProduct.tag}
          </p>
...
export default ProductDetailsIndigo;
